...
const discordObject = require('../../utils/discord-bot');
...
module.exports = {
    ...
    create: async (req, res, next) => {
        try {
            ...
            const eventData = await noteService.create(userId, data);
    ...
    delete: async (req, res, next) => {
        try {
            const userId = req.user.id;
            const noteId = req.params.id;
            const eventData = await noteService.delete(userId, noteId);
            res.status(204).json();
        } catch (error) {
            next(error);
        }
